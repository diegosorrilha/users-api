...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		msg := fmt.Sprintf("Error to update user with id %d: %v", id, err)
		log.Print(msg)
		responses.FailResponse("InternalServerError", resp, w)
		return
	}
	...
	if rows > 0 {
		resp = map[string]any{
			"message": fmt.Sprintf("User updated with success. id: %d", id),
		}

	} else {
		msg := fmt.Sprintf("No record has been updated. id: %d", id)
		log.Print(msg)
		resp = map[string]any{
			"message": msg,
		}
